...
void sobe(theap *heap, int i) {
    int p;
    // O loop continua enquanto o nó não for a raiz
    while (i > 0) {
        p = pai(i);

        // Para uma MAX-HEAP, o filho deve ser MAIOR que o pai para subir
        if (heap->lista[i].distancia > heap->lista[p].distancia) {
            troca(&heap->lista[i], &heap->lista[p]);
            // Continua a verificação a partir da nova posição (do pai)
            i = p;
        } else {
            // Se o filho é menor ou igual ao pai, ele está na posição correta.
            // O loop DEVE parar aqui.
            break;
        }
    }
}
